...
class ScrapeController
{
    ...
    public function scrape(Request $request, Response $response): Response
    {
        ...
        $stmt = $this->pdo->query("SELECT p.id, p.name, p.url, p.current_price, p.threshold, ph.price_history FROM products p LEFT JOIN (SELECT product_id, JSON_ARRAYAGG(price) AS price_history FROM price_history GROUP BY product_id) ph ON p.id = ph.product_id");
        $products = $stmt->fetchAll();
